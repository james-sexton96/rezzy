...
Deno.test("LatexCoverLetterRenderer - renders complete cover letter", () => {
  const renderer = new LatexCoverLetterRenderer(sampleResume, sampleCoverLetter);
  ...
  assertEquals(resultString.includes("\\documentclass[12pt,letterpaper]{article}"), true);
  ...
  assertEquals(resultString.includes("I am writing to apply for the Software Developer position."), true);
...
Deno.test("LatexCoverLetterRenderer - handles missing resume basics", () => {
  ...
  const renderer = new LatexCoverLetterRenderer(incompleteResume, sampleCoverLetter);
  const result = renderer.render();
...
Deno.test("LatexCoverLetterRenderer - handles missing location in resume", () => {
  ...
  const renderer = new LatexCoverLetterRenderer(resumeWithoutLocation, sampleCoverLetter);
  const result = renderer.render();
...
Deno.test("LatexCoverLetterRenderer - handles empty profiles array", () => {
  ...
  const renderer = new LatexCoverLetterRenderer(resumeWithEmptyProfiles, sampleCoverLetter);
  const result = renderer.render();
