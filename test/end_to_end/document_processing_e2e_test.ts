...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      console.warn(
        `Skipping E2E test: Test PDF file not found at ${TEST_PDF_PATH}`,
      );
      return;
    }
    ...
    try {
      ...
      const resume = await processDocumentWithOpenAI(TEST_PDF_PATH) ||
        {} as ResumeSchema;
      ...
      assertEquals(
        ...
        true,
        "Resume should have basics section",
      );

      // Check that basic information was extracted
      if (resume.basics) {
        assertEquals(
          typeof resume.basics.name,
          "string",
          "Name should be a string",
        );
        assertEquals(
          resume.basics.name.length > 0,
          true,
        ...
        if (resume.basics.email) {
          assertEquals(
            typeof resume.basics.email,
            "string",
            "Email should be a string",
          );
          assertEquals(
            resume.basics.email.includes("@"),
            true,
        ...
        if (resume.basics.summary) {
          assertEquals(
            typeof resume.basics.summary,
            "string",
            "Summary should be a string",
          );
          assertEquals(
            resume.basics.summary.length > 0,
            true,
      ...
      if (resume.work && resume.work.length > 0) {
        ...
        assertEquals(
          typeof workEntry.name,
          "string",
          "Company name should be a string",
        );
        assertEquals(
          workEntry.name.length > 0,
          true,
      ...
      if (resume.education && resume.education.length > 0) {
        ...
        assertEquals(
          typeof educationEntry.institution,
          "string",
          "Institution should be a string",
        );
        assertEquals(
          educationEntry.institution.length > 0,
          true,
      ...
      if (resume.skills && resume.skills.length > 0) {
        ...
        assertEquals(
          typeof skillEntry.name,
          "string",
          "Skill name should be a string",
        );
        assertEquals(
          skillEntry.name.length > 0,
          true,
