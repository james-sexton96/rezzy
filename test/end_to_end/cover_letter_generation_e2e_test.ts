...
Deno.test({
  ...
  fn: async () => {
    ...
    if (!apiKey || !model) {
      console.warn(
        "Skipping E2E test: OPENAI_API_KEY or OPENAI_MODEL environment variables not set",
      );
      return;
    }
    ...
    try {
      ...
      assertEquals(
        typeof coverLetter.greeting,
        "string",
        "Greeting should be a string",
      );
      assertEquals(
        coverLetter.greeting.length > 0,
        true,
        "Greeting should not be empty",
      );

      assertEquals(
        typeof coverLetter.companyStreetAddress,
        "string",
        "Company street address should be a string",
      );
      assertEquals(
        typeof coverLetter.companyCity,
        "string",
        "Company city should be a string",
      );
      assertEquals(
        typeof coverLetter.companyState,
        "string",
        "Company state should be a string",
      );
      assertEquals(
        typeof coverLetter.companyZipCode,
        "string",
        "Company zip code should be a string",
      );

      assertEquals(
        typeof coverLetter.letterBody,
        "string",
        "Letter body should be a string",
      );
      assertEquals(
        coverLetter.letterBody.length > 100,
        true,
        "Letter body should be substantial",
      );

      // Check that the cover letter mentions the job and candidate's skills
      const letterText = `${coverLetter.greeting} ${coverLetter.letterBody}`
        .toLowerCase();
      assertEquals(
        letterText.includes("software developer"),
        true,
        "Cover letter should mention the job title",
      );
      assertEquals(
        letterText.includes("typescript") || letterText.includes("javascript"),
        true,
