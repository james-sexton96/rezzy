...
export class OllamaProvider implements LlmProvider {
  constructor(
    ...
    private readFileFn: (path: string) => Promise<Uint8Array> = Deno.readFile,
  ) {
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are an experienced resume cover letter writer. Respond ONLY with a valid JSON object for the cover letter.",
      },
      {
        role: "user",
        content: [
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a resume parser. Extract information from the attached PDF and respond ONLY with a valid JSON Resume object.",
      },
      {
        role: "user",
        content:
          "Please extract the resume information from the attached PDF document. Respond with a JSON Resume object.",
      },
      {
        role: "user",
    ...
    const messages = [
      {
        role: "system",
        content:
          "You are a resume parser. Extract information from the following plain text resume and respond ONLY with a valid JSON Resume object.",
      },
      {
        role: "user",
        content: resumeText,
