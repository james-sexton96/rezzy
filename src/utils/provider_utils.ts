...
export function validateOpenAIEnv(): { isValid: boolean; message?: string } {
  ...
  if (!validModels.includes(env.OPENAI_MODEL)) {
    return {
      isValid: false,
      message:
        `The specified model '${env.OPENAI_MODEL}' may not be valid or available. Valid models include: ${
          validModels.join(", ")
        }. If you're using a newer model that's not in this list, you can ignore this warning.`,
    };
  }
